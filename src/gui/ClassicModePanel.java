...
import static gui.Screen.db;
import static gui.Screen.selectedPlayer;
import static main.Main.BoardXCoordinate;
import static main.Main.BoardYCoordinate;

public class ClassicModePanel extends JPanel {
	...
	public ClassicModePanel() {
		...
		//Mode name
		modeName = new JLabel("Classic Mode");
		modeName.setFont(new Font("Comic Sans MS",Font.BOLD,56));
		modeName.setForeground(Color.BLACK);
		modeName.setBounds(790,100,1600,50);
		this.add(modeName);

		//scroll pane adding buttons
		levelsPanel = new JPanel(null);
		levelsPanel.setOpaque(false);
		int counter = 1;
		int xCounter = 25;
		int yCounter = 0;
		int lastUnlockedLevel = Integer.parseInt(db.getLastUnlockedLevel(player).substring(db.getLastUnlockedLevel(player).length() - 1));
		for(char[][] c: db.getAllLevels())
		{

			JButton j = new JButton(); //Create a button for each level
			j.setText(counter + "");
			if (counter <= lastUnlockedLevel)
				j.setEnabled(true);
			else
				j.setEnabled(false);
			...
			j.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					int[] walls = {1,1,1,1,0,0};
					Screen.getInstance().setActivePage("PlayBoard",c,1,walls);
				}
			} );
			counter++;
			levelsPanel.add(j);
		...
		for(String s: Screen.db.getAllThemes())
		{
			...
			JRadioButton j = new JRadioButton(s); //Create a JButton for every theme
